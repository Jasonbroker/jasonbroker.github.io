...
首先，谈到运行时，就得谈下c语言，不得不提到compiler，传统面向过程的c compiler逻辑是比较简单的，但是compiler的优化是非常难的，本人完全不懂，就不讨论了。compiler最基础的逻辑是把函数名称转化成一个相对的内存地址，把函数内的语句转化成jmp指令，运行程序就能调用正确的语句找到函数。
...
`-isMemberOfClass: `是 `NSObject` 的方法，用以确定某个 NSObject 对象是否是某个类的成员。与之相似的为 -isKindOfClass:，可以用以确定某个类是否是某个类或其子类。这两个方法为典型的introspection方法。在确定对象为某类成员后，可以安全地进行强制转换，继续之后的工作，因此这里的安全判断又是一种运行时的体现。
    ...
    这里的`-isMemberOfClass: `以及`isKindOfClass:`本身是如何实现的呢？
    这里提出一个新的概念叫做isa指针（isa pointer）。
    要说isa就有太多内容要讲了，因此我把这部分放到另外一篇文章中讨论。
...
他把年龄设置为20岁！
ok了，然后底层c语言怎么干的呢？
先找到p所在的类，然后找到这个方法的实现：
（注意：这里面p的类是在运行时动态产生的，不一定是Person类，在我后面的KVO中会仔细讲这个问题）；
        setAge：
然后调用这个方法并为他传值20。
也就是写为
        ...
        - (void)encodeWithCoder:(NScoder *)coder
        {
            ...
            for (int i = 0; i <count; i++){
                ...
                const char *propertyName = ivar_getName(ivar);
                    ...
                    [coder encodeObject:[self valueForKeyPath:propertyName] forKey:key];
                }
        }
